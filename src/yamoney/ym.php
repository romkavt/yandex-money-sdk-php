...
interface IYandexMoney {
     ...
     * TOKEN_STORAGE_FILE. Но будьте внимательны: если вы запускате примеры на хостинге, 
     * то программная запись файлов в каталог по умолчанию скорее всего запрещена. 
...
class YandexMoney implements IYandexMoney {
    ...
    public function storeToken($key, $accessToken) {
        $encryptedToken = base64_encode(mcrypt_ecb(MCRYPT_3DES, $key, $accessToken, MCRYPT_ENCRYPT));
    ...
    public function restoreToken($key) {
        if (file_exists(self::TOKEN_STORAGE_FILE)) {
            ...
            if (array_key_exists($key, $tokenArray)) {
                $decryptedToken = mcrypt_ecb(MCRYPT_3DES, $key, base64_decode($tokenArray[$key]), MCRYPT_DECRYPT);
